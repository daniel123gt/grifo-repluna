                ...
                <div>
                    ...
                    <nav className="text-gray-light pt-8 flex flex-col gap-4 text-xl">
                        {linksRoutes.map((item, index) => (
                            <NavLink key={index} to={item.href} end={item.isEnd} className={({ isActive, isPending, isTransitioning }) =>
                                [
                                  isPending ? "pending" : "",
                                  isActive ? "text-primary-green" : "",
                                  isTransitioning ? "transitioning" : "",
                                ].join(" block mb-2 font-light text-lg ")
                              }><i className={item.icon}></i><span className="pl-6">{item.text}</span></NavLink>
                        ))}
                    </nav>
                </div>
                <button className="outline-none bg-secondary-green hover:opacity-85 transition text-white w-full h-[56px] rounded-sm cursor-pointer"><i className="fa-solid fa-right-from-bracket"></i><span className="pl-4">Cerrar Sesión</span></button>
            </aside>
